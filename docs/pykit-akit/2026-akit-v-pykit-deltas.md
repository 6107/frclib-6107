...
> both projects' in-progress/unreleased work (see `2027-akit-v-pykit-deltas.md`
...
- PyKit has **no equivalent** for several `Logger` utility methods present in AdvantageKit v26.0.2: `runEveryN`,
  ...
  `../lib_6107-api-work-todo.md` — this document confirms the gap traces all the way back to upstream PyKit, it isn't
...
| `runEveryN(int n, Runnable function)`                                              | —                                                                                  | **Gap.** No decimation helper; teams must hand-roll a modulo counter (as Team 4572 Barlow Robotics does — see `2026-telemetry-examples.md` §4.6).                                      |
...
| `ReplayWatch` — standalone CLI tool (`public static void main(...)`) that watches a directory and (per its file-visitor logic) reacts to new/changed log files, used for auto-triggering replay/AdvantageScope workflows | — (not present in the `v1.0.5` release tree; a "replay watch on its own project" commit exists only on the unmerged, out-of-scope `origin/real-hardware` branch — see `2027-akit-v-pykit-deltas.md` §3) | **Gap at the released-version level.** Some equivalent capability may be under early development upstream (unreleased branch), but nothing shipped in `v1.0.5`.                                                                                                                                    |
...
per `akit-changes-to-date.md` — a newer `Systemcore` implementation on unreleased `main`) selected automatically by
...
in its public API; per the architecture notes already captured in `westwood-pykit-changes-to-date.md`, it instead
uses
...
documented in
`westwood-pykit-changes-to-date.md` as the sole change introduced in PyKit v1.0.5 itself). There is no PyKit
...
`2027-akit-v-pykit-deltas.md` §3, that branch is a stale, pre-1.0 experiment older than the current `main`/`v1.0.5`
...
- `akit-changes-to-date.md` — AdvantageKit's public API as of `main` (post-2027 work) and its delta from `v26.0.2`;
...
- `westwood-pykit-changes-to-date.md` — PyKit's public API "as of v1.0.5" and confirmation that `main` has had no
  commits since that tag; the primary existing source for PyKit's architecture notes referenced throughout §2.
- `2027-akit-v-pykit-deltas.md` — the WPILib-2027/SystemCore-specific comparison (a different, later-stage axis of
  comparison than this document, which is deliberately scoped to both projects' **last released** versions).
- `../lib_6107-api-work-todo.md` — prior `lib_6107.pykit`-specific gap list; this document confirms several of those
  gaps (e.g. `runEveryN`) originate upstream in PyKit itself, not from `lib_6107`'s own port.
- `2026-telemetry-examples.md` — real-world example of a team (4572 Barlow Robotics) hand-rolling a `runEveryN`
